...
class Script(object):
  ...
  def check_package_condition(package):
    ...
    sys.stderr.write("CARONTE")
    Logger.info("VIRGILIO")
    if not condition:
      return True
    ...
    try:
      ...
      raise Fail("Alessio: Condition with name '{0}', when installing package {1}. Please check package_conditions.py.".format(condition, name))
