...
def flink(service=None,action=None):
  import params
...
#    execute_hdfs(chown_jars)
  ...
  if action == "config":
    configurations = params.config['configurations']['flink']
    File(format(params.flink_conf_dir+"/flink-conf.yaml"),
       content=Template("flink-conf.yaml.j2",configurations=configurations),
